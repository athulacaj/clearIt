...
class CoinsClass {
  int coins = 0;
  static int staticCoins = 0;
  CoinsClass({required int coins}) {
    this.coins = coins;
    staticCoins = coins;
  }
}
...
class CoinsStream {
  ...
  FirebaseFirestore _fireStoreDataBase = FirebaseFirestore.instance;
  late StreamController<CoinsClass> controller;

  Stream<CoinsClass> get getCoin {
    controller = StreamController<CoinsClass>();
    if (uid != null) {
      return _fireStoreDataBase
          .collection('uid')
          .doc(uid)
      ...
      controller.add(CoinsClass(coins: 0));
